# Volcano Monitoring Dashboard - Class Diagram
...
classDiagram
    ...
    class AddEventOverlay {
        -Function(Map~String, dynamic~?) onSubmitted
    ...
    Event *-- EventAttachment : contains multiple
    Event *-- DiscussionMessage : contains multiple
    Event *-- GroupMember : may contain multiple
    ...
    NavigationBloc -.-> MyApp : state provider
    TimelineCubit -.-> TimelinePage : state provider
    MapCubit -.-> MapPage : state provider
    ComparisonBloc -.-> ComparisonPage : state provider
    ComparisonBloc -.-> ComparisonResultsPage : state provider
    EventVisibilityCubit -.-> EventVisibilityPanel : state provider
